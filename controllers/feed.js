...
exports.createPost = async (req, res, next) => {
  ...
  try {
    ...
    await user.save();
    io.getIO().emit('posts', { action: 'create', post: { ...post._doc, creator: { _id: req.userId, name: user.name } } });    // "emit" will send to all connected clients but "broadcast" sends data to all clients.
    res.status(201).json({ message: 'Post created successfully!', post: post, creator: { _id: user._id, name: user.name } });
  } catch (err) {
    if (!err.statusCode) {
      err.statusCode = 500;
    }
